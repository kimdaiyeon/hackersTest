		...
		<?if($now_page == 1){
			while($row_best = mysql_fetch_array($result_B)){?>
				<tr class="bbs-sbj">
					...
					<td>
						<a href="/lecture_board/index.php?mode=view&boardNo=<?=$row_best['no']?>&now_page=<?=$now_page?>">
			...
			<?}
				...
				<tr class="bbs-sbj">
					...
					<td><?=$row['lectype'] ?></td>
					<td>
						<a href="/lecture_board/index.php?mode=view&boardNo=<?=$row['no']?>&now_page=<?=$now_page?>">
...
<div class="box-paging">
	<? include_once "{$_SERVER['DOCUMENT_ROOT']}/lecture_board/view/include/BoardPage.php"; ?>
</div>